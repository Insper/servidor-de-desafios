...
class Tutorial(Exercicio):
    ...
    def descricao_html(self):
        ...
        resultado = markdown.markdown(self.descricao, extensions=['codehilite'])
        com_div = ''
        slide_id = 1
...
class AcessoAoTutorial(models.Model):
    primeiro_acesso = models.DateTimeField('primeiro acesso', auto_now_add=True)
    ...
    def __str__(self):
        return '{0}: Tutorial {1} Primeiro Acesso[{2}] Último Acesso[{3}] Total de Acessos[{4}]'.format(self.usuario.username, self.tutorial.id, self.primeiro_acesso, self.ultimo_acesso, self.total_acessos)
