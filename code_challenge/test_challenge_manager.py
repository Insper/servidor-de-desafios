...
@pytest.mark.asyncio
async def test_list_new_trace_challenges(tmp_path):
    git = gitpy.Git(tmp_path)
    await git.init()
    n = 5
    fs = {}
...
'''
            ...
            files = save_trace_challenge(tmp_path, challenge_name, details, code, trace)
...
'''
        ...
        for f in files:
            await git.add(f)
        challenge_paths.append(tmp_path / 'traces' / challenge_name)
    await git.commit(f'-m "Adding all files."')
